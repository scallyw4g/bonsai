...
DeserializeCurrentVersion(u8_cursor *Bytes, noise_layer_1 *Element, memory_arena *Memory)
{
  ...
  if (ThisMember == False)
  {
    SoftError("Deserializing brush_noise_type Type on noise_layer_1");
  }
  Result &= ThisMember;
  ThisMember = 3;
  ...
  if (ThisMember == False)
  {
    SoftError("Deserializing white_noise_params White on noise_layer_1");
  }
  Result &= ThisMember;
  ThisMember = 3;
  ...
  if (ThisMember == False)
  {
    SoftError("Deserializing perlin_noise_params Perlin on noise_layer_1");
  }
  Result &= ThisMember;
  ThisMember = 3;
  ...
  if (ThisMember == False)
  {
    SoftError("Deserializing voronoi_noise_params Voronoi on noise_layer_1");
  }
  Result &= ThisMember;
