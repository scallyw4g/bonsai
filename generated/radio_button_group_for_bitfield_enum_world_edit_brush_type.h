...
GetRadioEnum(ui_toggle_button_group *RadioGroup, world_edit_brush_type *Result)
{
  if (RadioGroup->ToggleBits)
  {
    ...
    Assert(((RadioGroup->ToggleBits == WorldEditTool_BrushType_Asset||RadioGroup->ToggleBits == WorldEditTool_BrushType_Entity||RadioGroup->ToggleBits == WorldEditTool_BrushType_Noise)));
    /* Assert((((enum_t.map(value).sep(|) {value.name})) & RadioGroup->ToggleBits) != 0); */
  }

  *Result = Cast(world_edit_brush_type, RadioGroup->ToggleBits);
}
...
RadioButtonGroup_world_edit_brush_type(renderer_2d *Ui, window_layout *Window, const char *ToggleGroupIdentifier, ui_toggle_button_group_flags ExtraFlags = ToggleButtonGroupFlags_None, UI_FUNCTION_PROTO_DEFAULTS)
{
  cs ButtonNames[] =
  {
    CSz("Asset"),
