...
link_internal world_edit_block_array_index
Capacity( world_edit_block_array *Arr )
{
  world_edit_block_array_index Result = {Arr->BlockCount * 8};
  return Result;
}
...
{
  world_edit_block_array_index Result = {Arr->ElementCount};
  return Result;
}
...
GetBlock( world_edit_block_array *Arr, world_edit_block_array_index Index )
{
  umm BlockIndex   = Index.Index / 8;
  Assert(BlockIndex < Arr->BlockCount);
  world_edit_block *Block = Arr->BlockPtrs[BlockIndex];
  return Block;
}
...
GetPtr( world_edit_block_array *Arr, world_edit_block_array_index Index )
{
  ...
  world_edit_block *Block = GetBlock(Arr, Index);
...
link_internal void
NewBlock( world_edit_block_array *Arr )
{
  world_edit_block  *NewBlock     = Allocate( world_edit_block , Arr->Memory,                 1);
  world_edit_block **NewBlockPtrs = Allocate( world_edit_block*, Arr->Memory, Arr->BlockCount+1);
...
link_internal void
RemoveOrdered( world_edit_block_array *Array, world_edit_block_array_index IndexToRemove)
{
  Assert(IndexToRemove.Index < Array->ElementCount);

  world_edit *Prev = {};

  world_edit_block_array_index Max = AtElements(Array);
  RangeIteratorRange_t(umm, Index, Max.Index, IndexToRemove.Index)
  {
    world_edit *E = GetPtr(Array, Index);
...
Push( world_edit_block_array *Array, world_edit *Element)
{
  ...
  world_edit *Result = Set(Array, Element, AtElements(Array));
