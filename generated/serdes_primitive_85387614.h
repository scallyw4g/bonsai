...
{
  b32 Result = Write(Bytes, Cast(u8*, Element), sizeof(chunk_flag));
  return Result;
}

link_internal b32
Deserialize(u8_cursor *Bytes, chunk_flag *Element, memory_arena *Ignored = 0)
{
...
link_internal b32
SerializeArray(u8_cursor_block_array *Bytes, chunk_flag *Element, umm Count)
{
  Assert(Count);
  {
...
link_internal b32
DeserializeArray(u8_cursor *Bytes, chunk_flag **Dest, umm Count, memory_arena *Memory)
{
  Assert(Count);
