...
AllocateGpuElementBuffer(gpu_element_buffer_handles *Handles, data_type Type, u32 ElementCount)
{
  ...
  switch (Type)
  {
    ...
    case DataType_v3_u8:
    {
      ...
      GL.BindBuffer(GL_ARRAY_BUFFER, Handles->VertexHandle);
      GL.BufferData(GL_ARRAY_BUFFER, v3u8Size, 0, GL_STATIC_DRAW);
      AssertNoGlErrors;
...
MapGpuBuffer_untextured_3d_geometry_buffer(gpu_element_buffer_handles *Handles)
{
  ...
  switch (Handles->ElementType)
  {
    ...
    case DataType_v3:
    {
      ...
      GL.BindBuffer(GL_ARRAY_BUFFER, Handles->VertexHandle);
      AssertNoGlErrors;
      Buffer.Verts = (v3*)GL.MapBufferRange(GL_ARRAY_BUFFER, 0, BufferSize, GL_MAP_WRITE_BIT);
      AssertNoGlErrors;

      GL.BindBuffer(GL_ARRAY_BUFFER, Handles->NormalHandle);
      AssertNoGlErrors;
      Buffer.Normals = (v3*)GL.MapBufferRange(GL_ARRAY_BUFFER, 0, BufferSize, GL_MAP_WRITE_BIT);
      AssertNoGlErrors;

      // Color data
      GL.BindBuffer(GL_ARRAY_BUFFER, Handles->MatHandle);
      AssertNoGlErrors;
      Buffer.Mat = (matl*)GL.MapBufferRange(GL_ARRAY_BUFFER, 0, MaterialBufferSize, GL_MAP_WRITE_BIT);
      AssertNoGlErrors;
    ...
    case DataType_v3_u8:
    {
      ...
      GL.BindBuffer(GL_ARRAY_BUFFER, Handles->VertexHandle);
      AssertNoGlErrors;
      Buffer.Verts = (v3_u8*)GL.MapBufferRange(GL_ARRAY_BUFFER, 0, BufferSize, GL_MAP_WRITE_BIT);
      AssertNoGlErrors;

      GL.BindBuffer(GL_ARRAY_BUFFER, Handles->NormalHandle);
      AssertNoGlErrors;
      Buffer.Normals = (v3_u8*)GL.MapBufferRange(GL_ARRAY_BUFFER, 0, BufferSize, GL_MAP_WRITE_BIT);
      AssertNoGlErrors;
