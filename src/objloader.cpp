...
{
  u32 *Elements = Allocate(u32, Memory, Count , True);
...
{
  v3 *Elements = Allocate(v3, Memory, Count , True);
...
ReadUntilTerminatorList(stream_cursor *Cursor, const char *TerminatorList, memory_arena *Arena)
{
  ...
  char *Result = Allocate(char, Arena, ResultLength + 1, True);
...
LoadObj(memory_arena *PermMem, const char * FilePath)
{
  ...
  model Result = {};
  Result.Chunk = Allocate(chunk_data, PermMem, 1, True);;
