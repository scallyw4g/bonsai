...
AllocateAndInitSsaoNoise(ao_render_group *AoGroup, memory_arena *GraphicsMemory)
{
  ...
  v3 *SsaoNoise = PUSH_STRUCT_CHECKED(v3, GraphicsMemory, 16, True);
...
{
  ao_render_group *Result = PUSH_STRUCT_CHECKED(ao_render_group, Mem, 1, True);
...
{
  g_buffer_render_group *gBuffer = PUSH_STRUCT_CHECKED(g_buffer_render_group, Memory, 1, True);
...
InitGbufferRenderGroup( g_buffer_render_group *gBuffer, memory_arena *GraphicsMemory)
{
  ...
  gBuffer->Textures = PUSH_STRUCT_CHECKED(g_buffer_textures, GraphicsMemory, 1, True);
...
{
  game_lights *Lights = PUSH_STRUCT_CHECKED(game_lights, GraphicsMemory, 1, True);
  Lights->Lights      = PUSH_STRUCT_CHECKED(light, GraphicsMemory, MAX_LIGHTS, True);
...
UpdateLightingTextures(game_lights *Lights)
{
  ...
  v3 *PosData = PUSH_STRUCT_CHECKED(v3, TranArena, MAX_LIGHTS, True);
  v3 *ColorData = PUSH_STRUCT_CHECKED(v3, TranArena, MAX_LIGHTS, True);
...
{
  graphics *Result = PUSH_STRUCT_CHECKED(graphics, GraphicsMemory, 1, True);
  Result->Memory = GraphicsMemory;

  Result->Lights = LightingInit(GraphicsMemory);

  Result->Camera = PUSH_STRUCT_CHECKED(camera, GraphicsMemory, 1, True);
  InitCamera(Result->Camera, 1000.0f);

  shadow_render_group *SG = PUSH_STRUCT_CHECKED(shadow_render_group, GraphicsMemory, 1, True);
