...
TestChunkCopy(memory_arena *Memory)
{
  ...
  { // Ensure the flat noise initialization works correctly
    ...
    InitializeChunkWithNoise( Terrain_Flat, &MainThread, DestChunk, DestChunkDim, 0, 0, 0, 0, GRASS_GREEN, MeshBit_Lod0, ChunkInitFlag_Noop, 0);
  ...
  { // Ensure copy works as intended
    ...
    InitializeChunkWithNoise( Terrain_Flat, &MainThread, SyntheticChunk, SyntheticChunk->Dim, 0, 0, 0, 0, GRASS_GREEN, MeshBit_Lod0, ChunkInitFlag_Noop, 0);
